...
static void homework2_test_internal(heap_t* heap, fs_t* fs, bool use_compression)
{
	...
	const char* smaller_example = "Hello, world. Today is a beautiful day. From Evan. YOU don't know about me without you have read a book by the name of The Adventures of Tom Sawyer; but that ain't no matter.  That book was made by Mr. Mark Twain, and he told the truth, mainly.";
	const size_t huck_finn_len = strlen(huck_finn);
	const size_t smaller_example_len = strlen(smaller_example);

	const char* write_data = smaller_example;
	fs_work_t* write_work = fs_write(fs, "foo.bar", write_data, smaller_example_len, use_compression);
	...
	assert(fs_work_get_size(write_work) == smaller_example_len);

	char* read_data = fs_work_get_buffer(read_work);
	assert(read_data && strcmp(read_data, smaller_example) == 0);
	assert(fs_work_get_result(read_work) == 0);
	assert(fs_work_get_size(read_work) == smaller_example_len);
	
	fs_work_destroy(read_work);
	fs_work_destroy(write_work);

	//heap_free(heap, read_data); Access violoation here
}
